...
export const MagicLinkEmail = ({ siteName, confirmationUrl }: Props) => (
  <Html lang="pt" dir="ltr">
    <Head />
    <Preview>O seu link de acesso à {siteName}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={brandBar}><Text style={brand}>{siteName}</Text></Section>
        <Heading style={h1}>Link de acesso</Heading>
        <Text style={text}>Clique abaixo para entrar na {siteName}. Este link expira em breve.</Text>
        <Button style={button} href={confirmationUrl}>Entrar</Button>
        <Text style={footer}>Se não pediu este link, ignore este email.</Text>
      </Container>
    </Body>
  </Html>
)
